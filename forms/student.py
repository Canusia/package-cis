...
class StudentVerifyEmailForm(forms.Form):
    """
    Student Form - Initial email verification step.
    ...
    class Media:
        js = ('js/form_validation.js',)

    first_name = with_meta(
        forms.CharField(
            label='Legal Student First Name',
            max_length=128,
    ...
    email = with_meta(
        forms.EmailField(
            ...
            help_text='DO NOT USE YOUR SCHOOL EMAIL ADDRESS',
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            form_labels = json.loads(form_settings.get('verify_form_field_messages', '{}'))
    ...
    def save(self, commit=True):
        ...
        # Build user kwargs from fields with target='user'
        user_kwargs = {'username': data.get('email')}  # username always matches email
        for field_name, field in self.fields.items():
            target = getattr(field, 'storage_target', None)
            if target == 'user':
                path = getattr(field, 'storage_path', None) or field_name
                user_kwargs[path] = data.get(field_name)

        user = CustomUser(**user_kwargs)
        user.save()
        ...
        try:
            ...
            from cis.utils import active_term
            student.meta['ferpa_completed_for'] = []
            student.meta['state_q_completed_for'] = []
            student.meta['start_term'] = active_term().code if active_term() else '-'
    ...
    def clean_email(self):
        ...
        from cis.settings.signup import signup as signup_settings
        config = signup_settings.from_db()
        
        try:
            config = json.loads(config['error_messages'])
            config = config.get('start_app', {})
        except Exception as e:
            config = {}
        ...
        if user:
            ...
            try:
                ...
                if student.account_verified:
                    if student.user.psid in ['', '-']:
                        message = config.get('dup_email', {}).get('pending_ernie_login')
                    else:
                        message = config.get('dup_email', {}).get('being_processed')
                else:
                    message = config.get('dup_email', {}).get('account_unverified')
                    student.send_verification_request_email()
            except (Student.DoesNotExist, AttributeError):
                message = config.get('dup_email', {}).get('non_student_account_exists')
