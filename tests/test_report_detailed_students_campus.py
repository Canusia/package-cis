"""Campus gate for the detailed_students_with_class report: a ce requester's
export only includes registrations at their processable campuses (via
...
class DetailedStudentsReportCampusTests(TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _data(self, campus):
        return {'term': [str(self.term.id)], 'status': ['applied'],
                'campus': campus}
    ...
    def test_campus_field_is_required_multiselect_of_accessible(self):
        ...
        req.user = self.ce
        form = detailed_students_with_class(req)
    ...
    def test_ce_result_scoped_to_selected_own_campus(self):
        report = detailed_students_with_class()
