...
class DanglingTabTests(DanglingInstructorFixtureMixin, TestCase):
    def setUp(self):
        self.build_fixture()
        self.build_dangling()
        self.url = reverse('cis:instructors')

    def tearDown(self):
        self.tear_down_fixture()

    def test_dangling_tab_renders(self):
        resp = self.client.get(self.url)
        self.assertEqual(resp.status_code, 200)
        body = resp.content.decode()
        self.assertIn('href="#dangling"', body)
        self.assertIn('records_instructor_dangling', body)

    def test_tab_points_at_the_dangling_api(self):
        body = self.client.get(self.url).content.decode()
        self.assertIn('/ce/api/instructor-dangling?format=datatables', body)
    ...
    def test_tab_posts_to_the_dangling_endpoint(self):
        body = self.client.get(self.url).content.decode()
        self.assertIn(
            reverse('cis:instructor_do_dangling_bulk_action'), body)

    def test_existing_instructors_table_is_still_the_first_tab(self):
        body = self.client.get(self.url).content.decode()
