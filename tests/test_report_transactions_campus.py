"""Campus gate for the transactions report: a ce requester's export only
includes transactions for students registered at their processable campuses
(via student -> studentregistration -> class_section -> course -> campus),
...
from cis.reports.transactions import transactions
...
class TransactionsReportCampusTests(TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def _data(self, campus):
        return {'term': [str(self.term.id)], 't_type': ['payment'],
                'created_on_from': [None], 'created_on_until': [None],
                'campus': campus}
    ...
    def test_ce_result_scoped_to_selected_own_campus(self):
        ...
        self.assertIn(self.txn_a, qs)
        self.assertNotIn(self.txn_b, qs)

    def test_ce_cannot_widen_to_inaccessible_campus(self):
        # selecting A (accessible) + B (not) keeps only A's transactions.
        report = transactions()
        qs = report.get_result(
    ...
    def test_ce_only_inaccessible_selection_returns_none(self):
        ...
        self.assertNotIn(self.txn_a, qs)
        self.assertNotIn(self.txn_b, qs)
