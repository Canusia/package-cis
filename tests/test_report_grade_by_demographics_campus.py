"""Campus gate for the grade_by_demographics report: a ce requester's
pre-aggregation registrations queryset only includes registrations at their
processable campuses (via class_section -> course -> campus), optionally
...
from grades.reports.grade_by_demographics import grade_by_demographics
...
class GradeByDemographicsReportCampusTests(TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_campus_field_is_required_multiselect_of_accessible(self):
        ...
        req.user = self.ce
        form = grade_by_demographics(req)
