"""Campus gate for the student_imaging_export report: a ce requester's export
only includes students who registered at their processable campuses (via
...
from cis.reports.student_imaging_export import student_imaging_export
...
class StudentImagingExportReportCampusTests(TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def setUp(self):
        ...
        verified_on = datetime.datetime(2024, 1, 15)
        self.student_a = self._student_with_registration(self.sec_a, verified_on)
        self.student_b = self._student_with_registration(self.sec_b, verified_on)
    ...
    def _student_with_registration(self, section, verified_on):
        u = User.objects.create_user(
            username=f'stu_{_sfx()}', email=f'stu_{_sfx()}@x.com', password='x')
        student = Student.objects.create(
            user=u, account_verified=True, highschool=self.hs,
            account_verified_on=verified_on)
    ...
    def _data(self, campus):
        return {
            'account_verified_on_from': ['01/01/2024'],
            'account_verified_on_until': ['01/31/2024'],
    ...
    def test_campus_field_is_required_multiselect_of_accessible(self):
        ...
        req.user = self.ce
        form = student_imaging_export(req)
