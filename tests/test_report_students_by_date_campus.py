"""Campus gate for the students_by_date report: a ce requester's export only
includes students whose registrations are at their processable campuses
(via studentregistration -> class_section -> course -> campus), optionally
narrowed to a selected campus. Superusers are unscoped.
"""
import uuid
import datetime
...
from cis.reports.students_by_date import students_by_date
...
class StudentsByDateReportCampusTests(TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def setUp(self):
        ...
        # date range used by the report's user__created_at filter
        self.range_start = datetime.datetime(2024, 1, 1)
        self.range_end = datetime.datetime(2024, 2, 1)
        created_at_in_range = datetime.datetime(2024, 1, 15)

        self.student_a = self._student(self.sec_a)
        self.student_b = self._student(self.sec_b)

        # user.created_at is auto_now_add; force it into the report's window
        # after creation via a queryset update (bypasses auto_now_add).
        User.objects.filter(
            pk__in=[self.student_a.user_id, self.student_b.user_id]
        ).update(created_at=created_at_in_range)
    ...
    def _data(self, campus):
        return {
            'applied_on': [self.range_start.strftime('%m/%d/%Y')],
            'applied_until': [self.range_end.strftime('%m/%d/%Y')],
