"""Campus gate for the recommendation_export report: a ce requester's export
only includes recommendations for students registered at their processable
campuses (via student -> studentregistration -> class_section -> course ->
campus), optionally narrowed to a selected campus. Superusers are unscoped.
"""
import uuid
...
class RecommendationExportReportCampusTests(TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def setUp(self):
        ...
        self.rec_a = self._recommendation(self.sec_a)
        self.rec_b = self._recommendation(self.sec_b)
    ...
    def _recommendation(self, section):
        ...
        return StudentRecommendation.objects.create(
            student=student, term=self.term, recommendation={})

    def _data(self, campus):
        return {'term': [str(self.term.id)], 'campus': campus}

    def test_campus_field_is_required_multiselect_of_accessible(self):
        from django import forms as dj_forms
        req = RequestFactory().get('/')
        req.user = self.ce
        form = recommendation_export(req)
    ...
    def test_ce_result_scoped_to_selected_own_campus(self):
        report = recommendation_export()
        qs = report.get_result(
            self._data(campus=[str(self.campus_a.id)]), user=self.ce)
        self.assertIn(self.rec_a, qs)
        self.assertNotIn(self.rec_b, qs)

    def test_ce_cannot_widen_to_inaccessible_campus(self):
        # selecting A (accessible) + B (not) keeps only A's recommendations.
        report = recommendation_export()
        qs = report.get_result(
    ...
    def test_ce_only_inaccessible_selection_returns_none(self):
        ...
        self.assertNotIn(self.rec_a, qs)
        self.assertNotIn(self.rec_b, qs)
