...
from cis.validators import validate_email_list, validate_cron

class pending_class_syllabi(forms.Form):
    ...
    cron = forms.CharField(
        ...
        label="Cron Expression for emails to instructors pending syllabus",
        validators=[validate_cron]
    )

    pending_upload_email_header = FFields.ReadOnlyField(
        required=False,
        label='<h2>Emails to Instructors who have not uploaded syllabus.</h2>',
        initial='',
    ...
    email_message = forms.CharField(
        max_length=None,
        widget=forms.Textarea,
        help_text='Email template sent to instructor when missing class syllabi Short codes supported {{instructor_first_name}}, {{instructor_last_name}}, {{section_list}}, {{term}}. <a href="#" class="float-right" onClick="do_bulk_action(\'pending_class_syllabi\', \'email\')" >See Preview</a>'
    )
    ...
    needs_update_email_message = forms.CharField(
        max_length=None,
        widget=forms.Textarea,
        help_text='Email template sent to instructor when class syllabi is marked as \'Needs Update\' Short codes supported {{instructor_first_name}}, {{instructor_last_name}}, {{section_list}}, {{term}}. <a href="#" class="float-right" onClick="do_bulk_action(\'pending_class_syllabi\', \'needs_update_email\')" >See Preview</a>'
    )

    
    faculty_email_header = FFields.ReadOnlyField(
        required=False,
        label='<h2>Faculty Email Configuration - Sent to faculty with classes that have pending syllabus review</h2>',
        initial='',
        widget=FFields.LongLabelWidget(
            attrs={
                'class':'border-0 bg-light h-100'
    ...
    portal_headers = FFields.ReadOnlyField(
        required=False,
        label='<h2>Message displayed in instructor and faculty portal page(s)</h2>',
        initial='',
        widget=FFields.LongLabelWidget(
            attrs={
                'class':'border-0 bg-light h-100'
    ...
    teacher_email_header = FFields.ReadOnlyField(
        required=False,
        label='<h2>Instructor Email Configuration - Sent to instructor after syllabus has been reviewed</h2>',
        initial='',
        widget=FFields.LongLabelWidget(
            attrs={
                'class':'border-0 bg-light h-100'
    ...
    email_message = forms.CharField(
        max_length=None,
        widget=forms.Textarea,
        label='Needs Review - Message',
        validators=[validate_html_short_code],
        help_text='Email template sent to instructor after syllabi is marked as needs update. Short codes supported {{instructor_first_name}}, {{instructor_last_name}}, {{course_name}}, {{term}}, {{note}}. <a href="#" class="float-right" onClick="do_bulk_action(\'syllabi_review\', \'email_message\')" >See Preview</a>'
    )
    ...
    def clean_term(self):
        return str(self.cleaned_data.get('term').id)
